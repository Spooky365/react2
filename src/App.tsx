...
// +++ KOMPONENTE ZUR E-MAIL-MASKIERUNG +++
const ObfuscatedEmail = ({ user, domain }) => {
  const [email, setEmail] = useState('');

  useEffect(() => {
    // Dieser Effekt läuft nur im Browser des Clients.
    // Spam-Bots, die nur HTML parsen, sehen die Adresse nicht.
  ...
  return <a href={`mailto:${email}`} className="text-cyan-400 hover:underline">{email}</a>;
};

// +++ KOMPONENTE FÜR DAS IMPRESSUM +++
const ImpressumPage = ({ onBack }) => {
  return (
    <main className="text-gray-300 bg-slate-900/50">
      <div className="max-w-4xl mx-auto py-20 px-4 sm:px-6 lg:px-8">
        <button onClick={onBack} className="inline-flex items-center mb-12 text-cyan-400 hover:text-cyan-300 transition-colors group">
          <ArrowLeft className="h-5 w-5 mr-2 transition-transform group-hover:-translate-x-1" />
          Zurück zur Hauptseite
        </button>
        <div className="bg-slate-800/80 backdrop-blur-sm border border-slate-700/50 rounded-xl p-8 md:p-12 space-y-8">
...
// +++ KOMPONENTE FÜR DIE DATENSCHUTZERKLÄRUNG +++
...
function App() {
  ...
  const [view, setView] = useState('main'); // 'main', 'impressum', oder 'datenschutz'
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white">
      <nav className="fixed top-0 left-0 right-0 bg-slate-900/95 backdrop-blur-sm border-b border-slate-700/50 z-50">
          ...
          <div className="flex justify-between items-center h-12">
            ...
            <div className="hidden md:flex items-center space-x-8">
              {['home', 'services', 'about', 'projects', 'contact'].map((item) => (
                <a key={item} href={`#${item}`} onClick={(e) => { e.preventDefault(); scrollToSection(item); }} className={`text-sm font-medium transition-colors ${activeSection === item && view === 'main' ? 'text-cyan-400' : 'text-gray-300 hover:text-white'}`}>
        ...
        {isMenuOpen && (<div className="md:hidden bg-slate-800/95 backdrop-blur-sm border-t border-slate-700/50"><div className="px-2 pt-2 pb-3 space-y-1">{['home', 'services', 'about', 'projects', 'contact'].map((item) => (<a key={item} href={`#${item}`} onClick={(e) => { e.preventDefault(); scrollToSection(item); }} className="block px-3 py-2 text-base font-medium text-gray-300 hover:text-white hover:bg-slate-700/50 rounded-md w-full text-left">{item === 'home' ? 'Start' : item === 'services' ? 'Leistungen' : item === 'about' ? 'Über mich' : item === 'projects' ? 'Projekte' : 'Kontakt'}</a>))}</div></div>)}
      </nav>
            ...
            <div className="max-w-7xl mx-auto">
              <div className="text-center">
                ...
                <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white mb-6">
                  ...
                  <span className="block text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-400">für Ihr Unternehmen</span>
                </h1>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center space-y-4 sm:space-y-0 sm:space-x-6">
                  <button onClick={() => scrollToSection('contact')} className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-semibold rounded-lg hover:from-cyan-600 hover:to-blue-600 transition-all transform hover:scale-105 shadow-lg hover:shadow-xl">
                    Projekt besprechen
                    <ArrowRight className="ml-2 h-5 w-5" />
                  </button>
                  <button onClick={() => scrollToSection('projects')} className="inline-flex items-center px-8 py-4 bg-slate-800/80 text-white font-semibold rounded-lg hover:bg-slate-700 transition-all border border-slate-600">
                    Projekt-Highlight
                  </button>
                </div>
              </div>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {services.map((service, index) => (
                  <div key={index} className="bg-slate-900/80 backdrop-blur-sm border border-slate-700/50 rounded-xl p-8 hover:border-cyan-500/50 transition-all hover:transform hover:scale-105 group">
                    <div className="text-cyan-400 mb-6 group-hover:text-cyan-300 transition-colors">{service.icon}</div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                <div>
                  ...
                  <div className="mt-8 grid grid-cols-2 gap-6">
                    <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700/50 transition-all hover:transform hover:scale-105 hover:border-cyan-500/50"><div className="flex items-center text-2xl font-bold text-cyan-400 mb-1">24/7<Phone className="ml-2 h-6 w-6" /></div><div className="text-sm text-gray-400">erreichbar</div></div>
                    <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700/50 transition-all hover:transform hover:scale-105 hover:border-cyan-500/50"><div className="flex items-center text-2xl font-bold text-cyan-400 mb-1">100 %<Smile className="ml-2 h-6 w-6" /></div><div className="text-sm text-gray-400">kundenorientiert</div></div>
                  </div>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-8 border border-slate-700/50 transition-all hover:transform hover:scale-105 hover:border-cyan-500/50">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                    {[{ icon: <Globe className="h-5 w-5" />, name: "Cloud-Infrastruktur" }, { icon: <Cpu className="h-5 w-5" />, name: <>Docker<br />Kubernetes</> }, { icon: <HardDrive className="h-5 w-5" />, name: "Linux-Administration" }, { icon: <Code className="h-5 w-5" />, name: "Infrastructure as Code" }, { icon: <Zap className="h-5 w-5" />, name: "Ansible-Automatisierung" }, { icon: <Lock className="h-5 w-5" />, name: "Firewalls & Security" }, { icon: <GitFork className="h-5 w-5" />, name: "Loadbalancing" }, { icon: <Power className="h-5 w-5" />, name: "High Availability" }, { icon: <TrendingUp className="h-5 w-5" />, name: "Scaling" }].map((skill, index) => (<div key={index} className="flex items-center space-x-3 text-gray-300"><div className="text-cyan-400">{skill.icon}</div><span className="text-sm leading-tight">{skill.name}</span></div>))}
                  </div>
                </div>
              </div>
            </div>
          ...
          <section id="projects" className="py-20 px-4 sm:px-6 lg:px-8 bg-slate-800/50">
            <div className="max-w-7xl mx-auto">
              <div className="text-center mb-16">
                ...
                <p className="text-xl text-gray-300 max-w-3xl mx-auto">Einblick in eine vollautomatisierte CI/CD-Pipeline von der Entwicklung bis zum Live-System.</p>
              </div>
              <div className="flex justify-center">
                <div className="max-w-4xl w-full bg-slate-900/80 backdrop-blur-sm border border-slate-700/50 rounded-xl p-8 hover:border-cyan-500/50 transition-all hover:transform hover:scale-105">
                  <h3 className="text-2xl font-bold text-white mb-4">Vollautomatisierte CI/CD-Pipeline für eine React-Web-App</h3>
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">{['React', 'GitHub Actions', 'FluxCD', 'Terraform', 'Ansible', 'K3s', 'AWS'].map((tech) => (<span key={tech} className="px-3 py-1 bg-cyan-500/10 text-cyan-400 text-xs rounded-full border border-cyan-500/20">{tech}</span>))}</div>
                  <div className="bg-slate-800 p-4 rounded-lg border border-slate-600"><p className="text-green-400 font-semibold text-center">Resultat: Deployment von Zero-to-Live in wenigen Minuten und vollautomatisierte Updates.</p></div>
                </div>
              </div>
            </div>
          </section>
              ...
              <div className="flex justify-center">
                <div className="bg-slate-900/80 backdrop-blur-sm border border-slate-700/50 rounded-xl p-8 w-full max-w-xl">
                  <form onSubmit={handleFormSubmit} className="space-y-6">
                    ...
                    <div>
                      <button type="submit" disabled={formStatus === 'sending'} className="w-full inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-semibold rounded-lg hover:from-cyan-600 hover:to-blue-600 transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed">
